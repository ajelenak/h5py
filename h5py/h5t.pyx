...
from h5r cimport Reference, ExtReference, ExtRegionReference, \
                 RegionReference, AttributeReference, ExtAttributeReference
...
STD_REF_EXT_OBJ = lockid(H5T_STD_REF_EXT_OBJ)
STD_REF_EXT_REG = lockid(H5T_STD_REF_EXT_REG)
STD_REF_EXT_ATTR = lockid(H5T_STD_REF_EXT_ATTR)
...
cdef class TypeReferenceID(TypeID):
    ...
    cdef object py_dtype(self):
        ...
        elif H5Tequal(self.id, H5T_STD_REF_EXT_OBJ):
            return special_dtype(ref=ExtReference)
        elif H5Tequal(self.id, H5T_STD_REF_EXT_REG):
            return special_dtype(ref=ExtRegionReference)
        elif H5Tequal(self.id, H5T_STD_REF_EXT_ATTR):
            return special_dtype(ref=ExtAttributeReference)
        else:
...
cdef TypeReferenceID _c_ref(object refclass):
    if refclass is Reference:
        return STD_REF_OBJ
    elif refclass is ExtReference:
        return STD_REF_EXT_OBJ
    elif refclass is RegionReference:
        return STD_REF_REG
    elif refclass is ExtRegionReference:
        return STD_REF_EXT_REG
    elif refclass is AttributeReference:
        return STD_REF_ATTR
    elif refclass is ExtAttributeReference:
        return STD_REF_EXT_ATTR
...
def special_dtype(**kwds):
    ...
    ref = Reference | ExtReference | RegionReference | ExtRegionReference |
    ...
    if name == 'ref':
        ...
        if val not in (Reference, ExtReference, RegionReference,
                       AttributeReference, ExtRegionReference,
                       ExtAttributeReference):
            raise ValueError("Ref class must be (Ext)Reference or (Ext)RegionReference or (Ext)AttributeReference")
...
def check_dtype(**kwds):
    ...
    ref = dtype
        ...
        class (either (Ext)Reference or (Ext)RegionReference or
        (Ext)AttributeReference).  Returns None if the dtype does not represent
